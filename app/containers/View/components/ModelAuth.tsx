...
import { Table, Tabs, Radio, Checkbox, Select, Row, Col, Button, Tag, Tooltip, Icon,Input,Form } from 'antd'
const { Column } = Table
const { TabPane } = Tabs
...
import SetScheduleTime from './SetScheduleTime'
...
import {makeSelectEditingSchedule} from '../selectors'
...
import { ISchedule } from 'containers/Schedule/components/types'
...
interface IModelAuthProps {
  ...
  onViewChange?: (propName: string, value: string) => void
  view :any,
...
export class ModelAuth extends React.PureComponent<IModelAuthProps, IModelAuthStates> {
  ...
  private inputChange = (propName: keyof IView) => (event: any) => {
    this.props.onViewChange(propName, event.target.value)
  }
  ...
  public render() {
    const { visible, model, variable, viewRoles, sqlColumns, roles, onModelChange,view,
      editingSchedule = {
        cronExpression: '',
    ...
    const style = visible ? {paddingLeft: 20, paddingRight: 20} : { display: 'none' }
    const { name: viewName, description: viewDesc, sourceId, cronExpression = '', isManually = false } = view
    console.log(cronExpression, 'cronExpression')
    editingSchedule.cronExpression = cronExpression
    editingSchedule.setCronExpressionManually = isManually
        ...
        <Row>
          ...
          <FormItem label="名称" {...formItemLayout}>
              {getFieldDecorator('name', {
                ...
                <Input placeholder="名称" maxLength={30}  onChange={this.inputChange('name')} />
              )}
            </FormItem>
          ...
          <FormItem label="描述" {...formItemLayout}>
              {getFieldDecorator('description', {
                initialValue: viewDesc,
                // rules:[{
                //   required: true,
                //   message: '描述不能为空'
                // }]
              })(
                <Input.TextArea maxLength={100} placeholder="描述" autoSize={{minRows: 4}}  onChange={this.inputChange('description')} />
              )}
            </FormItem>
