...
import {
  ...
  IDacTenant,
  IDacBiz,
  IViewQueryResponse, StatisticsResult, IExcuteSqlStatisticsResponse, statisticsResponseData
} from './types'
...
export const ViewActions = {
  ...
  loadViews (projectId: number, tenantId?: number, page?: number, pageSize?: number, resolve?: (views: IViewBase[]) => void, rest?: any) {
  ...
  // 查询统计
  getStatisticsData(params: IExecuteSqlParams, exeType: EExecuteType) {
    return {
      type: ActionTypes.GET_STATISTICS_DATA,
      payload: {
        exeType,
        params
      }
  ...
  // 设置统计结果
  setStatisticsData(result: IExcuteSqlStatisticsResponse<statisticsResponseData>) {
    return {
      type: ActionTypes.SET_STATISTICS_DATA,
      payload: {
        result
      }
    }
  ...
  executeSql(params: IExecuteSqlParams, exeType: EExecuteType, isRunStatistics: boolean) {
    return {
