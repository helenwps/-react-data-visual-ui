...
import ProjectEditForm from './component/Project'
...
const Toolbar: React.FC<IToolbarProps> = React.memo(
  ({ pType, setPType, setKeywords, searchKeywords, showProForm }) => {
    ...
    const menus = useMemo(() => {
      const types = ['all', 'data', 'dataSource', 'join', 'create', 'favorite', 'history']
    ...
    return (
      <div className={styles.toolbar}>
        <div className={styles.menu}>
          ...
          <Link to="/project/6/views">
            <button>我的数据</button>
          </Link>
          <Link to="/project/6/sources">
            <button>数据源</button>
          </Link>
...
> = React.memo(
  ({
    ...
    const ProjectItems: ReactElement[] = useMemo(() => {
      const items = Array.isArray(projects)
        ? getProjectsBySearch.map((pro: IProject, index) => {
          ...
          console.log("isHistoryType", isHistoryType)
    ...
    let refProjectEditForm
    const afterProjectEditFormClose = () => {
      refProjectEditForm.props.form.resetFields()
    }

    let currentProject = {}
    const tabsChange = (mode) => {
      if (mode === 'basic') {
        const {
          orgId,
          id,
        ...
        refProjectEditForm.props.form.setFieldsValue({
          orgId: `${orgId}`,
          id,
          name,
          pic,
    ...
    const showProjectForm = (type: string) => (e) => {
      // e.stopPropagation()
      // this.setState({
      //   formVisible: true,
      //   formType: type
    ...
    const showEditProjectForm = (formType, option) => (e) => {
                ...
                <div style={{ overflow: 'hidden' }}>
                  ...
                  <div className={styles.flex}>
                    {ProjectItems.filter((item) => {
                      return item.props.owntype == "true"
                    })}
                  </div>
                </div>
                <div>
                  授权我的项目
                  <div className={styles.flex}>{ProjectItems.filter((item) => {
                  return item.props.owntype == "false"
                })}</div>
                </div>
              </div>
            ) : (
