...
export function* getSchedules (action: ScheduleActionType) {
  ...
  try {
    ...
    const rawSchedules = asyncData.payload
    const schedules = rawSchedules.list.map((schedule) => ({ ...schedule, config: JSON.parse(schedule.config) }))
    const total = rawSchedules.total
    yield put(ScheduleActions.schedulesLoaded(schedules, total))
  } catch (err) {
...
export function* getVizsData (action) {
  ...
  try {
    const portalsData = yield call(request, {
      url: `${api.portal}`,
      method: 'POST',
      data:  {projectId }
    })
