...
interface IDisplayFormModalProps {
  ...
  onSave: (display: Display, type: string, resolve: () => void) => void
  onCancel: () => void
...
export class DisplayFormModal extends React.PureComponent<IDisplayFormModalProps & FormComponentProps, {}> {
  ...
  private onModalOk = () => {
    const { type, display, projectId, onSave, form } = this.props
    form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        onSave({ ...display, ...values, projectId }, type, () => form.resetFields())
      }
    })
  }
