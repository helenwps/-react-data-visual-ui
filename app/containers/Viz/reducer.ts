...
const vizReducer = (
  ...
  produce(state, (draft: IVizState) => {
    ...
    switch (action.type) {
      case ActionTypes.RELEASE_GET_LIST:
        draft.releaseModel.tableList = []
        draft.releaseModel.loading = true
        break
      case ActionTypes.RELEASE_GET_LIST_SUCCESS:
        draft.releaseModel.loading = false
        draft.releaseModel.tableList = action.payload.dataList
        break
      case ActionTypes.RELEASE_GET_LIST_Fail:
        draft.releaseModel.loading = false
        draft.releaseModel.tableList = []
        break
      ...
      case ActionTypes.LOAD_PORTALS_SUCCESS:
        ...
        draft.portals = action.payload.result?.result || []
        draft.portalsTotal = action.payload.result?.total || 0
      ...
      case ActionTypes.EDIT_PORTAL_SUCCESS:
        ...
        draft.loading.editing = false
        break
      case ActionTypes.COPY_PORTAL:
        draft.loading.editing = true
        break
      case ActionTypes.COPY_PORTAL_SUCCESS:
        draft.loading.editing = false
        break
      case ActionTypes.COPY_PORTAL_FAILURE:
        draft.loading.editing = true
        break
      case ActionTypes.EDIT_PORTAL_FAILURE:
        draft.loading.editing = true
        break
      case ActionTypes.RELEASE_DASHBOARD:
        draft.loading.releaseing = true
        break
      case ActionTypes.RELEASE_DASHBOARD_SUCCESS:
        draft.loading.releaseing = false
        break
      case ActionTypes.RELEASE_DASHBOARD_FAILURE:
        draft.loading.releaseing = false
        break
      ...
      case ActionTypes.ADD_DISPLAY_SUCCESS:
        draft.displays.list.unshift(action.payload.result)
        draft.loading.editing = false
        break
      case ActionTypes.EDIT_DISPLAY_SUCCESS:
        draft.displays.list.splice(
          draft.displays.list.findIndex(({ id }) => id === action.payload.result.id),
          1,
          action.payload.result
        )
        if (
      ...
      case ActionTypes.COPY_DISPLAY_SUCCESS:
        draft.displays.list.unshift(action.payload.display)
        break
      ...
      case ActionTypes.DELETE_DISPLAY_SUCCESS:
        draft.displays = draft.displays.list.filter(
