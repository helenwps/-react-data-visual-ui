...
export function* getPortals(action: VizActionType) {
  ...
  try {
    const asyncData = yield call(
      request,
      {url: `${api.portalList}`, method: 'post', data: {projectId: payload.projectId, page, pageSize}}
    )
    const portals = asyncData.payload
    yield put(VizActions.portalsLoaded({
      total: portals.total,
      result: portals.list || []
    }))
  } catch (err) {
...
export function* editPortal(action: VizActionType) {
  ...
  try {
    yield call(request, {
      method: 'put',
      url: `${api.portalEdit}/${payload.values.id}`,
      data: payload.values
    })
...
export function* releaseDashboard(action: VizActionType) {
  if (action.type !== ActionTypes.RELEASE_DASHBOARD) {
    return
  }
  ...
  try {
    yield call(request, {
      method: 'post',
      url: `${api.releaseDashboard}`,
      data: params
    })
    yield put(VizActions.dashboardReleased())
    resolve()

  } catch (err) {
    yield put(VizActions.dashboardReleaseFail())
    errorHandler(err)
  }
}

export function* copyPortal(action: VizActionType) {
  if (action.type !== ActionTypes.COPY_PORTAL) {
    return
  }

  const { payload } = action
  const {name,description,publish = true,roleIds,id} = payload.values
  try {
    yield call(request, {
      method: 'post',
      url: `${api.portalCopy}/${id}`,
      data: {
        name,
        description,
        publish,
    ...
    yield put(VizActions.copySuccess())
    payload.resolve()
  } catch (err) {
    yield put(VizActions.copyFail())
    errorHandler(err)
  }
}
...
export function* getDisplays(action: VizActionType) {
  ...
  const { projectId, page, pageSize } = action.payload
  try {
    const asyncData = yield call(
      request,
      {
        url: `${api.display}/queryList`,
        method: 'post',
...
export function* getDisplaySlides(action: VizActionType) {
  ...
  try {
    ...
    slides && slides.forEach((slide: ISlideRaw) => {
...
export function* addSlide(action: VizActionType) {
  ...
  console.log(currentSlides, currentSlide)
...
export function* getReleaseList(action: VizActionType) {
  if (action.type !== ActionTypes.RELEASE_GET_LIST) {
    return
  }
  ...
  try {
    ...
    yield put(VizActions.getReleaseListSuccess(result?.payload))
  }
  catch (e) {
    yield put(VizActions.getReleaseListFail())
    errorHandler(e)
  }
}

export function* updateReleaseStatus(action: VizActionType) {
  if (action.type !== ActionTypes.RELEASE_OFFLINE) {
    return
  }
  const {status, dataId, resolve} = action.payload
  try {
    const result = yield call(request, {
      method: 'put',
      url: `${api.releaseUpdate}`,
      data: {status, id: dataId}
    })
    console.log(result, 'result')
    yield put(VizActions.updateReleaseStatusSuccess())
    resolve && resolve()
  }
  catch (e) {
    yield put(VizActions.updateReleaseStatusFail())
    errorHandler(e)
  }
}
...
export default function* rootVizSaga() {
  yield all([
    ...
    takeEvery(ActionTypes.COPY_PORTAL, copyPortal),
    ...
    takeEvery(ActionTypes.RELEASE_DASHBOARD, releaseDashboard),

    takeLatest(ActionTypes.RELEASE_GET_LIST, getReleaseList),

    takeLatest(ActionTypes.RELEASE_OFFLINE, updateReleaseStatus)
  ])
}
