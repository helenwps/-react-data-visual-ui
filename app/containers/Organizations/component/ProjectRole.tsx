...
interface IRoleProps {
  ...
  onUpdateRelRoleProject: (
    roleId: number,
    projectId: number,
    projectRole: object
  ) => any
  onDeleteRelRoleProject: (
    roleId: number,
    projectId: number,
    resolve?: () => any
  ) => () => any
...
export class ProjectRole extends React.Component<IRoleProps, IRoleStates> {
  ...
  private loadOrganizationRole = () =>{
    let currentOrganizationId = this.props.loginUser.orgId
    // if(!this.props.currentOrganization){
    //   currentOrganizationId = this.props.loginUser.orgId
    // }
    this.props.onLoadOrganizationRole(currentOrganizationId)
  }
  ...
  private toggleModal = (flag: string, id?: number) => () => {
    ...
    if (flag === 'relationRoleVisible') {
      this.setState({
        roleTargetKeys: this.state.copyTargetKeys,
  ...
  public componentWillReceiveProps(nextProps) {
    ...
    if (projectRoles !== this.props.projectRoles) {
      ...
      this.setState({ copyTargetKeys: roled })
    }
  ...
  private searchChange = (e) => {
    ...
    this.setState({
      searchValue,
      projectRoles:
        searchValue && searchValue.length ? result : this.props.projectRoles
    })
  }
  ...
  private setRoleTargetKeys = (newTargetKeys, callBack?) => {
    if (newTargetKeys) {
      this.setState({ roleTargetKeys: newTargetKeys }, callBack && callBack())
    }
  }
        ...
        render: (text, record) => {
          return (
            <span>
              <a
                href="javascript:;"
                onClick={this.toggleModal('authSettingVisible', record.id)}
              >
                权限设置
              </a>
              ...
              <Popconfirm
                title="确定删除？"
                placement="bottom"
                onConfirm={this.props.onDeleteRelRoleProject(
                  record.id,
                  projectDetail.id,
                  () => this.loadProjectRoles(projectDetail.id)
                )}
              >
                <Tooltip title="删除">
...
export function mapDispatchToProps(dispatch) {
  return {
    ...
    onAddProjectRole: (id, roleIds, resolve) =>
      dispatch(addProjectRole(id, roleIds, resolve)),
    onLoadRelRoleProject: (id, roleId) =>
      dispatch(loadRelRoleProject(id, roleId)),
    onUpdateRelRoleProject: (roleId, projectId, projectRole) =>
      dispatch(updateRelRoleProject(roleId, projectId, projectRole)),
    onDeleteRelRoleProject: (roleId, projectId, resolve) => () =>
      dispatch(deleteRelRoleProject(roleId, projectId, resolve)),
    onLoadVizVisbility: (roleId, projectId, resolve) =>
      dispatch(getVizVisbility(roleId, projectId, resolve)),
    onPostVizVisbility: (id, permission, reslove) =>
      dispatch(postVizVisbility(id, permission, reslove))
  }
}
