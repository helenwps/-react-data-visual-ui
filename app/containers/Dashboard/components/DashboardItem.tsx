...
import { Icon, Tooltip, Popconfirm, Popover, Dropdown, Menu, message } from 'antd'
...
import { htmlToImg } from 'containers/Dashboard/util'

export const MyCreate = React.createContext(null)
...
> {
  ...
  private dashboardItemElement: HTMLElement
  private getDashboardItemElement = (dom: HTMLElement) => {
      this.dashboardItemElement = dom
  }
  ...
  private downloadChartImg = async () => {
    try {
      await htmlToImg(this.dashboardItemElement.firstElementChild as HTMLElement)
    } catch (e) {
      message.error('下载失败！')
    }
  ...
  public render() {
    ...
    if (currentProject) {
      ...
      downloadButton = (
        <Tooltip title="下载数据">
          <div style={{display: 'inline-block', width: '38px', height: '30px'}}>
            <DownloadButton
      ...
      downloadImg = <Tooltip title="下载图片">
            <Icon type="download" onClick={this.downloadChartImg} />
        </Tooltip>
      ...
      <div className={gridItemClass} ref={(f) => (this.container = f)}>
        ...
        >
          <div className={styles.block}>
            <MyCreate.Provider value={this.getDashboardItemElement}>
              <Widget
                {...widgetProps}
