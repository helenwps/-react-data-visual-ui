...
export function* getDashboardDetail(action: DashboardActionType) {
  ...
  const { dashboardDetailLoaded, loadDashboardDetailFail, setDashboardTime } = DashboardActions
  ...
  try {
    ...
    if (items.length) {
      yield put(setDashboardTime(items[0].frequency))
    }
    ...
    const widgets: {formedWidgets:IWidgetFormed[]} = yield select(makeSelectWidgetsList())
    operationWidgetProps.widgetIntoPool(widgets.formedWidgets)
    ...
    yield put(dashboardDetailLoaded(dashboard, items, widgets.formedWidgets, formedViews))
  } catch (err) {
    console.log(err, 'err')
...
export function* addDashboardItems(action: DashboardActionType) {
  ...
  try {
    ...
    yield put(dashboardItemsAdded(result.payload, widgets.formedWidgets, formedViews))
...
export function* changeDashboardTime(action: DashboardActionType) {
  if (action.type !== ActionTypes.GOBLE_DASHBOARD_TIME) {
    return
  }
  const {
    dashboardId,
...
export function* getBatchDataWithControlValues(action: DashboardActionType) {
  ...
  if (type === ControlPanelTypes.Global) {
    ...
    console.log(currentDashboard, globalControlFormValues, 'value')
    ...
    console.log(globalControlConditionsByItem, 'globalControlConditionsByItem')
...
export function* downloadDashboardFile(action: DashboardActionType) {
  if (action.type !== ActionTypes.DOWN_DASHBOARD_FILE) {
    return
  }
  const {dashboardId, type} = action.payload
  console.log('start')
  try {
    const result = yield call(request, {
      method: 'get',
      url: `${api.downloadFile}/${dashboardId}/${type}`
    })
    let a = document.createElement("a");
    document.body.appendChild(a);
    a.href = result.payload;
    a.target = "_blank";
    a.download = result.payload.split("/")[result.payload.split("/").length - 1]
    a.click();
    a.remove();
    // location.href = result.payload
    yield put(DashboardActions.downloadDashboardFiled())
  } catch (err) {
    yield put(DashboardActions.downloadDashboardFiled())
    errorHandler(err)
  }
...
export default function* rootDashboardSaga() {
  yield all([
    ...
    takeEvery(ActionTypes.GOBLE_DASHBOARD_TIME, changeDashboardTime),
    takeEvery(ActionTypes.DOWN_DASHBOARD_FILE, downloadDashboardFile)
  ])
}
