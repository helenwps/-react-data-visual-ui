...
const makeSelectWidgetsList = () => createSelector(
  selectWidget,
  (widgetState) => widgetState.widgetsList
)
...
const makeSelectItemRelatedWidget = () =>
  createSelector(
    ...
    makeSelectItem(),
    (widgets, item) => {
      console.log(widgets, item)
      return widgets?.formedWidgets.find((w) => w.id === item.widgetId)
    }
  )
...
const makeSelectGlobalTime = () =>
  createSelector(
    selectDashboard,
    (dashboardState) => dashboardState.frequency
  )

const makeSelectDashboardDownloading = () =>
  createSelector(
    selectDashboard,
    (dashboardState) => dashboardState.downloadFileIng
  )
...
export {
  ...
  makeSelectDashboardDownloading
}
