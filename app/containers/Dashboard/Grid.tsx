...
import { makeSelectDownloadList, makeSelectLoginUser } from '../App/selectors'
...
import { ToolbarCopy } from 'containers/Dashboard/components/ToolbarCopy'
...
import { makeSelectWidgets, makeSelectWidgetsList } from 'containers/Widget/selectors'
...
import DashboardNav  from 'containers/Dashboard/components/DashboardNav'
import reducer from 'containers/Dashboard/reducer'
import saga from 'containers/Dashboard/sagas'
import DownloadList from 'components/DownloadList'
import { downloadFile, loadDownloadList } from 'containers/App/actions'
...
export class Grid extends React.Component<IGridProps & RouteComponentWithParams, IGridStates> {
  ...
  public componentWillMount () {
    const { match, widgetsList, onLoadDashboardDetail } = this.props
    const { portalId, dashboardId } = match.params
    if (dashboardId && Number(dashboardId) !== -1 && widgetsList?.formedWidgets.length) {
      onLoadDashboardDetail(+portalId, Number(dashboardId))
    }

    const reg = /^\/project\/\d+\/vizs\/portal\/\d+\/dashboard\/\d+\/preview[\/]*$/
    console.log(this.props.location.pathname, '1212')
  ...
  private loadViews = () => {
    const { match, onLoadViews, loginUser:{tenantId} } = this.props
  ...
  private saveDashboardItem = () => {
    const { match, currentDashboard, currentItems, widgetsList, formedViews, onLoadDashboardItemData } = this.props
    const portalId = +match.params.portalId
    ...
    const frequency = this.props.makeSelectGlobalTime
    const newItem = {
    ...
    if (dashboardItemFormType === 'add') {
      ...
      const selectedWidgetsViewIds = widgetsList?.formedWidgets.filter((w) => selectedWidgets.includes(w.id)).map((w) => w.viewId)
  ...
  private navToList = () => {
    ...
    const { projectId, portalId, dashboardId} = match.params
    if (isPreview) {
      this.props.history.push(`/project/${projectId}/vizs/portal/${portalId}/dashboard/${dashboardId}`)
      this.setState({
        isPreview: false
      })
    } else {
      this.props.history.push(`/project/${projectId}/vizs`)
    }
  }
  private navDropdownClick = (e) => {
    const { match } = this.props
    const { projectId, portalId } = match.params
    console.log('jump1111')
    this.props.history.push(`/project/${projectId}/vizs/portal/${portalId}/dashboard/${e.key}`)
  }
  ...
  private nextNavDropdownClick = (e) => {
    ...
    this.setState({
      nextMenuTitle: widgetsList.formedWidgets.find((widget) => widget.id === widgetId)['name']
    })
  }
  ...
  private getWidgetInfo = (dashboardItemId) => {
    const { currentItems, widgetsList } = this.props
    const dashboardItem = currentItems.find((ci) => ci.id === dashboardItemId)
    const widget = widgetsList.formedWidgets.find((w) => w.id === dashboardItem.widgetId)
    return {
      name: widget.name
    }
  }
  ...
  private setRefreshTime = (val)=> {
    const {onMakeSelectGlobalTime, match} = this.props
    onMakeSelectGlobalTime(val, Number(match.params.dashboardId), () => {
  ...
  private onPreview = ()=> {
    const { match } = this.props
    const { projectId, portalId, dashboardId} = match.params
    this.props.history.push(`/project/${projectId}/vizs/portal/${portalId}/dashboard/${dashboardId}/preview`)
    this.setState({
      isPreview: true
    })
  }
  private onRelease = ()=> {
    this.setState({isShowReleaseDialog: true})
  }
  private hideReleaseModel = () => {
    this.setState({isShowReleaseDialog: false})
  }
  private releaseSuccess = () => {
    this.hideReleaseModel()
  }
  public render () {
    const {
      dashboards,
    ...
    if (currentDashboard && widgetsList?.formedWidgets) {
      const navDropdownItems = currentItems.map((d) => {
        const wid = (widgetsList?.formedWidgets.find((widget) => widget.id === d.widgetId))
        return (
        <Menu.Item key={d.id}>
          {d.widgetId ?
    ...
    if (currentProject && currentItems && widgetsList?.formedWidgets) {
      const itemblocks = []
      const layouts = { lg: [] }
      ...
      currentItems.forEach((dashboardItem) => {
        ...
        const widget = widgetsList?.formedWidgets.find((w) => w.id === widgetId)
        ...
        itemblocks.push((
          <div key={id} className={styles.authSizeTag}>
            <DashboardItem
              itemId={id}
              alias={alias}
              widgets={widgetsList?.formedWidgets}
    ...
    const saveDashboardItemButton = (
      <Button
        ...
        disabled={modalLoading || !selectedWidgets.length}
    ...
    //     )]
    //   : saveDashboardItemButton
    const modalButtons = saveDashboardItemButton
    return (
      <Container>
        <DashboardNav isPreview={isPreview} title={currentDashboard && currentDashboard.name} refreshTime={makeSelectGlobalTime} setRefreshTime={this.setRefreshTime} navToList={this.navToList}>
          <ToolbarCopy
            isPreview={isPreview}
            currentProject={currentProject}
        ...
        {gridEditable && (
          <>
            <Modal
              title={`${dashboardItemFormType === 'add' ? '新增' : '修改'} 图表组件`}
        ...
        <PortalListReleaseModelForm
          ...
          releaseModal={{dashboardId: currentDashboard?.id}}
          hideReleaseModel={this.hideReleaseModel}
          operateSuccess={this.releaseSuccess}
          releaseDashboardIng={this.props.releaseDashboardIng}
          releaseDashboard={this.props.onReleaseDashboard}
        />
        <BottomOperate>
          {
            !this.state.isPreview && <Button onClick={this.onPreview}>预览</Button>
          }
          <Button type="primary" onClick={this.onRelease}>发布</Button>
        </BottomOperate>
      </Container>
    )
  }
...
const mapStateToProps = createStructuredSelector({
  ...
  widgetsList: makeSelectWidgetsList(),
  ...
  makeSelectGlobalTime: makeSelectGlobalTime(),
...
export function mapDispatchToProps (dispatch) {
  return {
    ...
    onLoadViews: (projectId: number, tenantId: number) => dispatch(loadViews(projectId, tenantId)),
    onLoadViewsDetail: (
      viewIds: number[],
    ...
    onMakeSelectGlobalTime: (frequency: number, dashboardId: number, resolve: () => void) => dispatch(changeDashboardTime(frequency, dashboardId, resolve)),
