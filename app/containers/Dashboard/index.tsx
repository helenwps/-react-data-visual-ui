...
export class Dashboard extends React.Component<IDashboardProps, IDashboardStates> {
  ...
  private initPortal = (projectId, portalId) => {
    ...
    onLoadDashboards(+portalId, (result) => {
      ...
      if (defaultDashboardId >= 0 && !dashboardId) {
        history.replace(`/project/${projectId}/vizs/portal/${portalId}/dashboard/${defaultDashboardId}`)
      }
  ...
  private changeDashboard = (dashboardId) => (e) => {
    ...
    this.setState({
      ...
      history.replace(`/project/${projectId}/vizs/portal/${portalId}/dashboard/${dashboardId}`)
    })
  }
          ...
          switch (formType) {
            ...
            // case 'copy':
              onAddDashboard(addObj, (dashboardId) => {
                ...
                const { projectId, portalId } = match.params
                addObj.type === 0
                  ? history.replace(`/project/${projectId}/vizs/portal/${portalId}`)
                  : history.replace(`/project/${projectId}/vizs/portal/${portalId}/dashboard/${dashboardId}`)
              })
              break
            case 'edit':
  ...
  public render () {
    ...
    return (
      <div className={styles.portal}>
        ...
        {/*        {*/}
        {/*          isGrid && widgetsList?.formedWidgets*/}
...
export function mapDispatchToProps (dispatch) {
  return {
    ...
    onLoadWidgets: (projectId: number) => dispatch(WidgetActions.loadWidgetsList(projectId)),
