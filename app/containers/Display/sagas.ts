...
export function* editSlideLayers (action: DisplayActionType) {
  ...
  console.log(layerParamsUnChanged)
  try {
...
export function* editSlideLayerParams (action: DisplayActionType) {
  ...
  console.log(action.payload.layerParamsUnChanged)
  ...
  yield put(DisplayActions.editSlideLayers(displayId, slideId, [updateLayer], action.payload.layerParamsUnChanged))
}
...
export default function* rootDisplaySaga () {
  yield all([
    ...
    // takeEvery(ActionTypes.CHANGE_TAB, changeTab)
  ])
}
