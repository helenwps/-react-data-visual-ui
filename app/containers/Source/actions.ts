...
import {
  ...
  ICSVMetaInfo,
  IDatabaseTables,
  IDatasourceInfo, ISourceResponse
} from './types'

export const SourceActions = {
  loadSources(tenantId: number, page?, pageSize?) {
    return {
  ...
  sourcesLoaded(sourceResponse: ISourceResponse) {
    return {
  ...
  deleteSource(id: number, resolve: () => void) {
    return {
  ...
  uploadCsvFile(csvMeta: ICSVMetaInfo, type, resolve: () => void, reject: () => void) {
    return {
