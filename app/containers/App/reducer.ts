...
const initialState = {
  ...
  menuActiveIndex: '0',
  isShowMenu: true,
  isShowProjectMenu: true,
  projectMenuActiveIndex: 'vizs'
}
  ...
  produce(state, (draft) => {
    switch (action.type) {
      ...
      case UPDATE_PROFILE_SUCCESS:
        const { id, name,email, phone,workNum, department } = action.payload.user
        draft.loginUser = {
          ...draft.loginUser,
          id,
          name,
      ...
      // 菜单相关
      case SHOW_MENU:
        draft.isShowMenu = true
        break
      case HIDE_MENU:
        draft.isShowMenu = false
        break
      case SET_MENU_ACTIVE:
        draft.menuActiveIndex = action.payload.menuActiveIndex
        break
      case SHOW_PROJECT_MENU:
        draft.isShowProjectMenu = true
        break
      case HIDE_PROJECT_MENU:
        draft.isShowProjectMenu = false
        break
      case SET_PROJECT_MENU_ACTIVE:
        draft.projectMenuActiveIndex = action.payload.projectMenuActiveIndex
        break
      // 菜单相关结束
