...
export function* login(action) {
  ...
  try {
    ...
    const loginUser = asyncData.payload
    //登录完成  只有租户的注册信息完善后才设置loginUser
    if((loginUser.userType != 2) || (loginUser.userType == 2 && loginUser.tenantId)){
      localStorage.setItem('loginUser', JSON.stringify(loginUser))
    }

    localStorage.setItem('UserRegisterTemp', JSON.stringify(loginUser))
...
export function* updateProfile(action) {
  const { id, name,email,phone,workNum, department, resolve } = action.payload
  ...
  try {
    ...
    const updateUserProfile = { id, name, email, phone,workNum, department}
...
export function* getCaptchaForResetPassword(
  ...
  const { type, ticket, resolve,reject } = action.payload
