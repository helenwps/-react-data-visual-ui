...
export function* getWidgets(action: WidgetActionType) {
  ...
  try {
    ...
    console.log(result)
    result.payload.list = result.payload.list.map(
      (widget: IWidgetRaw) => {
...
export function *getWidgetsList(action) {
  if (action.type !== ActionTypes.LOAD_WIDGETS_LIST) {
    return
  }
  const { projectId, page, pageSize } = action.payload
  try {
    ...
    const formedWidgets: IWidgetFormed[] = result.payload.list.map(
      (widget: IWidgetRaw) => {
        const parsedConfig = JSON.parse(widget.config)
        return {
          ...widget,
    ...
    yield put(WidgetActions.widgetsListLoaded({...result.payload, formedWidgets}))
  } catch (err) {
    yield put(WidgetActions.widgetsLoadedFail())
    errorHandler(err)
  }
...
export default function* rootWidgetSaga() {
  yield all([
    ...
    takeLatest(ActionTypes.LOAD_WIDGETS_LIST, getWidgetsList),
