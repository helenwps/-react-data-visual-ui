...
export function ChartIndicator (props: IChartIndicatorProps) {
  ...
  const renderIcon = (icon) => {
    const useTag = `<use xlink:href="#${icon}" />`
    console.log(checkChartEnable(dimetionsCount, metricsCount, chartInfo))
    return (
      <div className={iconClass} onClick={onSelect(props)}>
        <svg className="iconSvg" dangerouslySetInnerHTML={{__html: useTag }} />
      </div>
    )
  }
  return (
    <Tooltip
      ...
      mouseLeaveDelay={0}
    >
      {/* <i className={iconClass} onClick={onSelect(props)} style={{}} /> */}
      {renderIcon(icon)}
    </Tooltip>
  )
}
