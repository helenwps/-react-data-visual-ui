...
export class WidgetSelector extends React.Component<IWidgetSelectorProps, IWidgetSelectorStates> {
  ...
  private getWidgets () {
    ...
    console.log(widgets)
    if (!widgets || !Array.isArray(widgets.list)) {
      return []
    }

    const reg = new RegExp(kwWidget, 'i')

    const filteredWidgets = widgets.list.filter((w) => {
      let valid = true
      if (showSelected) {
  ...
  public render () {
    ...
    const widgetsList = widgetsFiltered.map((w, idx) => {
    ...
    console.log(widgetsList, widgetsCurrent)
